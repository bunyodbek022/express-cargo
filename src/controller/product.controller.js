...
export const Product = {
  ...
  async getOne(req, res, next) {
    try {
      const { id } = req.params
      const result = await baseClass.getOne('product', id)
      if (result === 400)
  ...
  async create(req, res, next) {
    try {
      ...
      const result = await baseClass.create('product', info)
      res.status(200).send({
        success: true,
        message: `product muvaffaqqiyatli yaratildi`,
        data: result,
      })
    } catch (err) {
      next(err)
  ...
  async update(req, res, next) {
    try {
      ...
      res.status(201).send({
        success: true,
        message: 'product muvaffaqqtiyatli yangilandi',
        data: result,
      })
    } catch (err) {
      next(err)
  ...
  async delete(req, res, next) {
    try {
      const { id } = req.params
      const result = await baseClass.delete('product', id)
      if (result === 400)
        return next(new ApiError(400, 'id UUID formatida bolishi kerak!'))
      if (result === 404)
        return next(new ApiError(404, 'Bu id dagi malumot topilmadi'))
      ...
      if (result === 200) {
        res.status(200).send({
          success: true,
          message: "product muvaffaqqtiyatli o'chirildi",
        })
      }
    } catch (err) {
      next(err)
